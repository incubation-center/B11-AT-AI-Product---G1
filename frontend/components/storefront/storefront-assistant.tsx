      ...
      {isOpen ? (
        <div className="fixed bottom-20 right-5 z-50 flex h-[min(72vh,640px)] w-[min(calc(100vw-2rem),390px)] flex-col overflow-hidden rounded-[28px] border border-slate-200 bg-white shadow-[0_28px_90px_rgba(15,23,42,0.18)]">
          ...
          <div className="shrink-0 border-t border-slate-200 bg-white px-4 py-4">
            ...
            <div className="flex items-center gap-2 rounded-3xl border border-slate-200 bg-slate-50 p-2 shadow-[inset_0_1px_0_rgba(255,255,255,0.8)]">
              <input
                value={input}
                ...
                className="h-12 flex-1 border-0 bg-transparent px-3 text-sm text-slate-900 caret-[#002e6b] outline-none placeholder:text-slate-400 focus:outline-none"
              />
              <Button
                type="button"
                onClick={() => void submitQuestion()}
                disabled={isPending || !input.trim()}
                className="h-12 w-12 rounded-2xl bg-[#002e6b] p-0 text-white hover:bg-[#003d8f]"
              >
