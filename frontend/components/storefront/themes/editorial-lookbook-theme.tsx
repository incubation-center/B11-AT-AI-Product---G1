    ...
    <section className="grid items-stretch gap-4 md:grid-cols-2 md:gap-8">
      <div className={reverse ? 'md:order-2' : ''}>
        <div className="h-full rounded-[26px] border border-[#ddd4c8] bg-[#fffcf6] p-6 md:p-8">
          <p className="text-[10px] font-semibold uppercase tracking-[0.2em] text-[#7a6146]">Lookbook Story</p>
          <h2 className="mt-3 text-3xl leading-tight text-[#231b14] md:text-4xl">{heading}</h2>
          <p className="mt-4 text-sm leading-7 text-[#5e5348] md:text-base">{body}</p>
        </div>
      </div>
      <div className={reverse ? 'md:order-1' : ''}>
        <div className="relative h-full min-h-64 overflow-hidden rounded-[26px] border border-[#ddd4c8] bg-[#e7ddcf]">
          <ProductImage src={imageUrl} alt={heading} sizes="(max-width: 768px) 100vw, 50vw" className="object-cover" />
        </div>
      </div>
    </section>
  );
  ...
  return (
    <section className="grid gap-4 md:grid-cols-3">
      {products.map((product) => (
        <article key={product.id} className="overflow-hidden rounded-2xl border border-[#ddd4c8] bg-white">
          <div className="relative aspect-[4/5] bg-[#f0ebe3]">
            <ProductImage src={product.imageUrls[0] ?? null} alt={product.name} sizes="(max-width: 768px) 100vw, 33vw" className="object-cover" />
          </div>
          <div className="space-y-2 p-4">
            <p className="text-[10px] font-semibold uppercase tracking-[0.18em] text-[#8a7257]">{product.category ?? 'Curated'}</p>
            <h3 className="line-clamp-1 text-base font-semibold text-[#231b14]">{product.name}</h3>
            <p className="text-sm font-semibold text-[#3f3125]">{formatStorePrice(product)}</p>
...
export function EditorialLookbookTheme({ store, products }: StorefrontThemeProps) {
        ...
        <header className="rounded-[30px] border border-[#ddd4c8] bg-[#fffcf6] p-6 md:p-8">
          <div className="flex items-center justify-between gap-4">
            <div className="flex items-center gap-3">
              ...
              />
              <p className="text-[10px] font-semibold uppercase tracking-[0.2em] text-[#7a6146]">{store.shopType}</p>
            </div>
            <div className="rounded-full border border-[#d8ccba] bg-white px-3 py-1 text-xs font-semibold text-[#5e5348]">
              Cart {totalItems}
            </div>
          </div>
          <h1 className="mt-5 text-4xl leading-tight md:text-6xl">Editorial / Lookbook</h1>
          <p className="mt-4 max-w-3xl text-sm leading-7 text-[#5e5348] md:text-base">
            {store.description ?? 'A premium story-driven layout with practical shopping actions in every strip.'}
          </p>
        </header>
      </section>

      <main id="products" className="mx-auto grid w-full max-w-[1360px] gap-8 px-4 pb-16 md:px-8 lg:px-12">
        <StoryBlock
        ...
        />
        {chunkA.length ? <ProductStrip products={chunkA} onSelect={setSelectedProduct} onAddToCart={(product) => addToCart(product, 1)} /> : null}
        ...
        <StoryBlock
          ...
          imageUrl={storySource[3]?.imageUrls[0] ?? storySource[1]?.imageUrls[0] ?? store.bannerUrl}
          reverse
        />
        {chunkB.length ? <ProductStrip products={chunkB} onSelect={setSelectedProduct} onAddToCart={(product) => addToCart(product, 1)} /> : null}
        ...
        <StoryBlock
          ...
          imageUrl={storySource[6]?.imageUrls[0] ?? storySource[2]?.imageUrls[0] ?? store.bannerUrl}
        />
        {chunkC.length ? <ProductStrip products={chunkC} onSelect={setSelectedProduct} onAddToCart={(product) => addToCart(product, 1)} /> : null}
      </main>
