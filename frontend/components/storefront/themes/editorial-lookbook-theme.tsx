    ...
    <section className="grid gap-4 md:grid-cols-3">
      {products.map((product) => (
        <article key={product.id} className="overflow-hidden rounded-2xl border border-[#ddd4c8] bg-white">
          <button type="button" onClick={() => onSelect(product)} className="w-full cursor-pointer text-left">
      ...
      <section className="mx-auto w-full max-w-[1360px] px-4 py-8 md:px-8 lg:px-12">
        <header className="rounded-[30px] border border-[#ddd4c8] bg-[#fffcf6] p-6 md:p-8">
          ...
          <p className="mt-4 max-w-3xl text-sm leading-7 text-[#5e5348] md:text-base">
            {store.description ??
              'Magazine-like storytelling with alternating narratives and curated product rows.'}
          </p>
        </header>
      </section>

      <main className="mx-auto grid w-full max-w-[1360px] gap-8 px-4 pb-16 md:px-8 lg:px-12">
        <StoryBlock
          heading={storySource[0]?.name ?? 'A New Seasonal Story'}
          body="Present your products as chapters, not just listings. This structure helps boutique brands build stronger emotional intent before checkout."
          imageUrl={storySource[0]?.imageUrls[0] ?? store.bannerUrl}
        />
        <ProductStrip
          products={chunkA.length ? chunkA : products.slice(0, 3)}
          onSelect={setSelectedProduct}
        />

        <StoryBlock
          heading={storySource[3]?.name ?? 'Craft, Material, and Mood'}
          body="Alternate imagery and editorial copy to create a premium browsing cadence. Every block adds context and increases perceived value."
          imageUrl={storySource[3]?.imageUrls[0] ?? storySource[1]?.imageUrls[0] ?? store.bannerUrl}
          reverse
        />
        <ProductStrip
          products={chunkB.length ? chunkB : products.slice(0, 3)}
          onSelect={setSelectedProduct}
        />

        <StoryBlock
          heading={storySource[6]?.name ?? 'Curated Finishing Picks'}
          body="Mix storytelling with concise product strips so buyers can feel the brand while still finding products quickly."
          imageUrl={storySource[6]?.imageUrls[0] ?? storySource[2]?.imageUrls[0] ?? store.bannerUrl}
        />
        <ProductStrip
          products={chunkC.length ? chunkC : products.slice(0, 3)}
          onSelect={setSelectedProduct}
        />
      </main>
