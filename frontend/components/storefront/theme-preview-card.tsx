...
function MinimalGridMock({ logoUrl }: { logoUrl?: string | null }) {
  return (
      ...
      <div className="grid h-full grid-cols-12 gap-2">
        ...
        <div className="col-span-12 grid grid-cols-4 gap-2">
          {Array.from({ length: 4 }).map((_, idx) => (
            <div
              key={idx}
              className="rounded-lg border border-[#e4e7ec] bg-white p-1.5"
            >
      ...
      <div className="grid h-full grid-cols-[1.25fr_0.75fr] gap-2">
        ...
        <div className="grid gap-2">
          {Array.from({ length: 3 }).map((_, idx) => (
            <div
              key={idx}
              className="rounded-lg border border-[#d1d8e7] bg-white"
            />
          ))}
        </div>
      </div>
...
function EditorialLookbookMock({
  bannerUrl,
}: {
  logoUrl?: string | null;
  bannerUrl?: string | null;
      ...
      <div className="flex h-full flex-col gap-2">
        ...
        <div className="grid flex-1 grid-cols-3 gap-2">
          {Array.from({ length: 3 }).map((_, idx) => (
            <div
              key={idx}
              className="rounded-lg border border-[#dccfbf] bg-white p-1.5"
            >
  ...
  return (
    <article
      className={cn(
        ...
        selected
          ? 'border-[#002e6b] ring-2 ring-[#002e6b]/20'
          : 'border-slate-200 hover:-translate-y-0.5 hover:border-slate-300',
      )}
    >
      {option.id === 'modern-minimal-grid' ? (
        <MinimalGridMock logoUrl={logoUrl} />
      ) : null}
      {option.id === 'hero-category-tiles' ? (
        <HeroCategoryMock logoUrl={logoUrl} bannerUrl={bannerUrl} />
      ) : null}
      {option.id === 'editorial-lookbook' ? (
        <EditorialLookbookMock bannerUrl={bannerUrl} />
      ) : null}

      <div className="mt-4 space-y-2">
        <div className="flex items-start justify-between gap-3">
          <div>
            <p className="text-[10px] font-semibold uppercase tracking-[0.16em] text-[#c61c2f]">
              {shopName}
            </p>
            <h3 className="text-lg font-semibold text-[#0f2345]">
              {option.name}
            </h3>
          </div>
          {selected ? (
        ...
        {onSelect ? (
          <button
            ...
            className={cn(
              ...
              selected
                ? 'bg-[#002e6b] text-white'
                : 'bg-[#ebf1ff] text-[#11336d] hover:bg-[#d9e5ff]',
            )}
          >
