...
export default function TelegramMiniAppClient() {
  ...
  const [storeAssetType, setStoreAssetType] = useState<'logo' | 'banner'>(
    'logo',
  );
  ...
  async function loadAll(nextToken: string) {
    try {
      ...
      const bootstrap = await api<Dashboard>(
        '/telegram/miniapp/bootstrap',
        nextToken,
      );
      const productData = await api<{ data: Product[] }>(
        '/telegram/miniapp/products?include_inactive=true&page_size=12',
        nextToken,
      );
      const orderData = await api<{ orders: Order[] }>(
  ...
  async function uploadStoreAsset() {
    ...
    const payload = await api<{ upload: { publicUrl: string } }>(
    ...
    setStoreForm((current) =>
      current
        ? {
            ...current,
            [storeAssetType === 'logo' ? 'logo_url' : 'banner_url']:
              payload.upload.publicUrl,
          }
        : current,
    );
    setStoreAsset(null);
  ...
  async function saveProduct() {
    ...
    startTransition(async () => {
      ...
      const endpoint = productForm.id
        ? `/telegram/miniapp/products/${productForm.id}`
  ...
  async function uploadProductImage() {
    ...
    const result = await api<{ image_urls: string[] }>(
      `/telegram/miniapp/products/${productForm.id}/images`,
      token,
      {
        method: 'POST',
        body: form,
    ...
    setProductForm((current) => ({
      ...current,
      image_urls: result.image_urls.join(', '),
    }));
  ...
  async function deactivateProduct() {
    ...
    await api(
      `/telegram/miniapp/products/${productForm.id}/deactivate`,
      token,
      { method: 'PATCH' },
    );
  ...
  async function openOrder(orderId: string) {
    if (!token) return;
    const detail = await api<{ order: Order }>(
      `/telegram/miniapp/orders/${orderId}`,
      token,
    );
  ...
  async function updateOrderStatus(statusValue: string) {
    ...
    const result = await api<{ order: Order }>(
      `/telegram/miniapp/orders/${selectedOrder.id}/status`,
      token,
      {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
  ...
  async function updateOrderPayment(paymentStatus: string) {
    ...
    const result = await api<{ order: Order }>(
      `/telegram/miniapp/orders/${selectedOrder.id}/payment`,
      token,
      {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
  ...
  async function cancelOrder() {
    ...
    const result = await api<{ order: Order }>(
      `/telegram/miniapp/orders/${selectedOrder.id}/cancel`,
      token,
      {
        method: 'POST',
      },
  ...
  return (
    <main className="min-h-screen bg-[linear-gradient(180deg,_#f7f1e7,_#efe4ce_45%,_#faf6ef)] px-3 py-4 text-stone-900">
      <div className="mx-auto max-w-6xl overflow-hidden rounded-[30px] border border-stone-900/10 bg-white/90 shadow-[0_25px_80px_rgba(64,41,15,0.12)]">
          ...
          <div className="mt-3 flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
            <div>
              ...
              <p className="mt-1 text-sm text-amber-50/80">
                {dashboard?.tenant.subdomain}.store | {status}
              </p>
            </div>
        ...
        <div className="grid gap-4 px-4 py-5 md:grid-cols-[1fr_1.4fr]">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="rounded-2xl bg-white/8 p-3">
                  ...
                  <div className="mt-2 text-2xl font-semibold">
                    {dashboard?.stats.products.total ?? 0}
                  </div>
                </div>
                <div className="rounded-2xl bg-white/8 p-3">
                  <div className="text-xs uppercase text-stone-400">Orders</div>
                  <div className="mt-2 text-2xl font-semibold">
                    {dashboard?.stats.orders.total ?? 0}
                  </div>
                </div>
                <div className="rounded-2xl bg-white/8 p-3">
                  <div className="text-xs uppercase text-stone-400">
          ...
          <section className="space-y-4">
            {tab === 'dashboard' && (
              <DashboardPanel dashboard={dashboard} onOpenOrder={openOrder} />
            )}
            {tab === 'store' && storeForm && (
              <section className="rounded-[28px] border border-stone-200 bg-white p-5">
                ...
                <div className="mt-4 grid gap-3 md:grid-cols-2">
                  <TextField
                    ...
                    onChange={(value) =>
                      setStoreForm({ ...storeForm, shop_name: value })
                    }
                  />
                  <TextField
                    label="Shop type"
                    value={storeForm.shop_type}
                    onChange={(value) =>
                      setStoreForm({ ...storeForm, shop_type: value })
                    }
                  />
                  <TextField
                    label="Address"
                    value={storeForm.address_text}
                    onChange={(value) =>
                      setStoreForm({ ...storeForm, address_text: value })
                    }
                  />
                  <TextField
                    label="Google Map URL"
                    value={storeForm.google_map_url}
                    onChange={(value) =>
                      setStoreForm({ ...storeForm, google_map_url: value })
                    }
                  />
                  <TextField
                    label="Logo URL"
                    ...
                    onChange={(value) =>
                      setStoreForm({ ...storeForm, logo_url: value })
                    }
                  />
                  <TextField
                    label="Banner URL"
                    ...
                    onChange={(value) =>
                      setStoreForm({ ...storeForm, banner_url: value })
                    }
                  />
                </div>
                ...
                <textarea
                  ...
                  className="mt-2 min-h-24 w-full rounded-2xl border border-stone-200 bg-stone-50 px-4 py-3 text-sm"
                />
                ...
                <div className="mt-6 rounded-2xl border border-stone-200 bg-stone-50 p-4">
                  ...
                  <div className="mt-3 flex flex-wrap gap-3">
                    <select
                      value={storeAssetType}
                      onChange={(event) =>
                        setStoreAssetType(
                          event.target.value as 'logo' | 'banner',
                        )
                      }
                    ...
                    <input
                      ...
                      accept="image/*"
                      onChange={(event) =>
                        setStoreAsset(event.target.files?.[0] ?? null)
                      }
                      className="rounded-2xl border border-stone-200 bg-white px-4 py-3 text-sm"
                    />
                    <button
                      type="button"
                      onClick={() => void uploadStoreAsset()}
                      className="rounded-2xl bg-amber-600 px-4 py-3 text-sm font-medium text-white"
                    >
                      Upload
                    </button>
                  </div>
                  ...
                  <div className="space-y-3">
                    {products.map((product) => (
                      ...
                      >
                        ...
                        <p className="mt-1 text-sm text-stone-600">
                          {product.category || 'No category'} | stock{' '}
                          {product.stockQty}
                        </p>
                      </button>
                    ))}
                  ...
                  <div>
                    <div className="grid gap-3 md:grid-cols-2">
                      ...
                      <TextField
                        ...
                        value={productForm.base_price_usd}
                        onChange={(value) =>
                          setProductForm({
                            ...productForm,
                            base_price_usd: value,
                          })
                        }
                      />
                      <TextField
                        label="Base price KHR"
                        value={productForm.base_price_khr}
                        onChange={(value) =>
                          setProductForm({
                            ...productForm,
                            base_price_khr: value,
                          })
                        }
                      />
                      <TextField
                        ...
                        onChange={(value) =>
                          setProductForm({ ...productForm, stock_qty: value })
                        }
                      />
                      <TextField
                    ...
                    <textarea
                      ...
                      className="mt-2 min-h-20 w-full rounded-2xl border border-stone-200 bg-stone-50 px-4 py-3 text-sm"
                    />
                    <div className="mt-3 grid gap-3 md:grid-cols-3">
                      ...
                      <Toggle
                        ...
                        onChange={(value) =>
                          setProductForm({ ...productForm, is_active: value })
                        }
                      />
                    </div>
                    <div className="mt-4 flex flex-wrap gap-3">
                  ...
                  <div className="rounded-2xl border border-stone-200 bg-stone-50 p-4">
                    {selectedOrder ? (
                      <>
                        ...
                        <p className="mt-1 text-sm text-stone-600">
                          {selectedOrder.customerName} | {selectedOrder.total}{' '}
                          {selectedOrder.currency}
                        </p>
                        <div className="mt-4 grid gap-3 md:grid-cols-2">
                          {['confirmed', 'delivering', 'completed'].map(
                            (item) => (
                              <button
                                key={item}
                                type="button"
                                onClick={() => void updateOrderStatus(item)}
                                className="rounded-2xl bg-stone-900 px-4 py-3 text-sm font-medium text-white"
                              >
                                {item}
                              </button>
                          ...
                          {['paid', 'unpaid'].map((item) => (
                            <button
                              key={item}
                              type="button"
                              onClick={() => void updateOrderPayment(item)}
                              className="rounded-2xl bg-amber-600 px-4 py-3 text-sm font-medium text-white"
                            >
                              mark {item}
                            </button>
                      ...
                      <p className="text-sm text-stone-600">
                        Select an order to inspect and update it.
                      </p>
                    )}
                  </div>
                </div>
      ...
      {isPending ? (
        <div className="fixed bottom-4 right-4 rounded-full bg-stone-900 px-4 py-2 text-xs font-medium text-white">
          Working...
        </div>
      ) : null}
    </main>
  ...
  return (
    <section className="rounded-[28px] border border-stone-200 bg-white p-5">
      ...
      <p className="mt-3 text-sm leading-6 text-stone-700">
        {dashboard?.tenant.description || 'No store description yet.'}
      </p>
      <div className="mt-4 grid gap-3 md:grid-cols-2">
        {dashboard?.recentOrders.map((order) => (
          <button
            ...
            className="rounded-2xl bg-stone-100 px-4 py-3 text-left"
          >
            <div className="flex items-center justify-between">
              <span className="font-medium">{order.orderNo}</span>
              <span className="text-sm uppercase text-stone-500">
            ...
            <p className="mt-1 text-sm text-stone-600">
              {order.total} {order.currency} | {order.paymentStatus}
            </p>
          </button>
        ))}
      </div>
