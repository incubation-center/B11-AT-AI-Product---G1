...
export const ordersApi = {
  getAll: () =>
    protectedFetch<{ orders: Order[] }>("/orders"),
  
  getById: (id: string) =>
    protectedFetch<Order & { items: any[]; payments: any[] }>(`/orders/${id}`),
...
export const inventoryApi = {
  getLowStockItems: async (): Promise<LowStockResponse> => {
    const response = await protectedFetch<BackendLowStockResponse>("/inventory/low-stock");
    ...
    return {
      items: (response.items ?? []).map((item) => ({
        ...
        low_stock_threshold: item.low_stock_threshold ?? item.lowStockThreshold ?? 0,
      })),
