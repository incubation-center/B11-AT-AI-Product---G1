...
export function StorefrontAssistant({ store }: StorefrontAssistantProps) {
  ...
  useEffect(() => {
    const stored = window.localStorage.getItem(`${STORAGE_KEY_PREFIX}.${store.subdomain}`);
    if (stored) {
      setSessionId(stored);
    }
  }, [store.subdomain]);
  ...
  const submitQuestion = async () => {
    ...
    try {
      ...
      const data = (await response.json()) as { message?: string } & Partial<AskAssistantResponse>;
      if (!response.ok || !data.answer || !data.session_id) {
        throw new Error(data.message ?? "Unable to ask assistant");
      }

      setSessionId(data.session_id);
      window.localStorage.setItem(`${STORAGE_KEY_PREFIX}.${store.subdomain}`, data.session_id);
      setMessages((current) => [
        ...current,
        {
      ...
      const message = submitError instanceof Error ? submitError.message : "Unable to ask assistant";
      setError(message);
